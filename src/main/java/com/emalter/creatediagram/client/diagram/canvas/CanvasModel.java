...
public class CanvasModel {
	...
	final float MIN_ZOOM = 0.2f, MAX_ZOOM = 3.0f;
	...
	int resizeMouseStartX = 0, resizeMouseStartY = 0;
	...
	protected NodeController nodeController = null;
	...
	public void setStrokes(List<DiagramStroke> loadedStrokes) { this.strokes.clear(); if (loadedStrokes != null) this.strokes.addAll(loadedStrokes); }
	...
	public DiagramNode findNode(UUID id) { for (DiagramNode n : nodes) if (n.id().equals(id)) return n; return null; }
	...
	public boolean isPositionValid(int targetX, int targetY, int targetW, int targetH, UUID ignoreId) {
		for (DiagramNode node : nodes) {
			...
			if (targetX < node.x() + node.width() && targetX + targetW > node.x() && targetY < node.y() + node.height() && targetY + targetH > node.y()) return false;
		}
		return true;
	}
