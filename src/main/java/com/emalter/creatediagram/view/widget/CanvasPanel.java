...
public class CanvasPanel {
    ...
    private final TextCommentEditor textEditor = new TextCommentEditor();

    private String previewItemType = null;

    public static record DiagramStroke(UUID id, int color, List<int[]> points) {}
    private final List<DiagramStroke> strokes = new ArrayList<>();
    private List<int[]> currentStrokePoints = null;

    private int lineStartX = 0, lineStartY = 0;
    private int lineCurrentX = 0, lineCurrentY = 0;
    ...
    private DiagramNode draggedNode = null;
    private int dragStartX = 0;
    private int dragStartY = 0;

    private DiagramNode resizingNode = null;
    ...
    private UUID nodeWithOpenMenu = null;
    private UUID nodeWithOpenColorMenu = null;

    private EditBox activeAmountField = null;
    private UUID nodeWithActiveAmountField = null;
    ...
    private DiagramNode findNode(UUID id) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick, int screenWidth, int screenHeight, int paletteWidth) {
        ...
        for (DiagramStroke stroke : strokes) drawStroke(guiGraphics, stroke);

        if (toolbar.getCurrentTool() == DrawingToolbar.Tool.PEN && currentStrokePoints != null && currentStrokePoints.size() > 1) {
            drawStroke(guiGraphics, new DiagramStroke(null, toolbar.getCurrentColor(), currentStrokePoints));
        }
        if (toolbar.getCurrentTool() == DrawingToolbar.Tool.LINE && isDrawingLine) {
            drawFastLine(guiGraphics, lineStartX, lineStartY, lineCurrentX, lineCurrentY, toolbar.getCurrentColor());
        }

        this.connectionManager.render(guiGraphics, this.nodes, worldX, worldY);
        for (DiagramNode node : nodes) drawNode(guiGraphics, node);

        if (nodeWithOpenMenu != null) {
            guiGraphics.pose().pushPose();
            guiGraphics.pose().translate(0, 0, 3);
            drawCatalystMenu(guiGraphics, nodeWithOpenMenu);
            guiGraphics.pose().popPose();
        }

        if (nodeWithOpenColorMenu != null) {
            guiGraphics.pose().pushPose();
            guiGraphics.pose().translate(0, 0, 3);
            drawNodeColorMenu(guiGraphics, nodeWithOpenColorMenu);
            guiGraphics.pose().popPose();
        }

        if (activeAmountField != null && nodeWithActiveAmountField != null) {
            guiGraphics.pose().pushPose();
            guiGraphics.pose().translate(0, 0, 4);
            activeAmountField.render(guiGraphics, (int)worldX, (int)worldY, partialTick);
            guiGraphics.pose().popPose();
        }
        ...
        boolean tooltipDrawn = this.connectionManager.renderTooltips(guiGraphics, mouseX, mouseY, worldX, worldY, nodes, this.font);

        if (!tooltipDrawn && nodeWithOpenMenu == null && nodeWithOpenColorMenu == null && activeAmountField == null && !textEditor.isEditing()) {
            for (DiagramNode node : nodes) {
        ...
        guiGraphics.drawString(this.font, "Zoom: " + Math.round(zoom * 100) + "%", 10, 10, 0xFFFFFFFF, true);
        guiGraphics.pose().popPose();
    }
    ...
    private void drawNode(GuiGraphics guiGraphics, DiagramNode node) {
        ...
        if (node.itemType().equals("creatediagram:text_comment")) {
            ...
            String displayText = textEditor.isEditing(node.id()) ? textEditor.getDisplayText() : node.property();
            guiGraphics.drawWordWrap(this.font, net.minecraft.network.chat.FormattedText.of(displayText), node.x() + 4, node.y() + 4, w - 8, 0xFF000000);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button, int paletteWidth) {
        ...
        if (textEditor.isEditing()) {
            DiagramNode editNode = findNode(textEditor.getEditingNodeId());
            if (editNode == null || !(worldX >= editNode.x() && worldX <= editNode.x() + editNode.width() && worldY >= editNode.y() && worldY <= editNode.y() + editNode.height())) {
        ...
        if (nodeWithOpenColorMenu != null) {
            DiagramNode node = findNode(nodeWithOpenColorMenu);
            if (node != null) {
                Color[] colors = Color.values();
                int menuX = node.x() + node.width() + 20;
                int menuY = node.y() + (node.height() / 2) - 8;
                if (worldX >= menuX && worldX <= menuX + 20 && worldY >= menuY && worldY <= menuY + (colors.length * 20)) {
                    ...
                    if (clickedIndex >= 0 && clickedIndex < colors.length) {
                        int nodeIdx = nodes.indexOf(node);
                        nodes.set(nodeIdx, new DiagramNode(node.id(), node.itemType(), node.x(), node.y(), node.property(), node.amount(), colors[clickedIndex].getHexValue(), node.width(), node.height()));
                        nodeWithOpenColorMenu = null;
                        return true;
                    }
                }
            }
        ...
        if (this.connectionManager.mouseClicked(worldX, worldY, this.nodes, button)) return true;

        if (button == 1) {
            boolean clickedOnNode = false;
            for (DiagramNode node : nodes) {
                if (worldX >= node.x() && worldX <= node.x() + node.width() && worldY >= node.y() && worldY <= node.y() + node.height()) {
                    this.connectionManager.removeNodeConnections(node.id());
                    nodes.remove(node);
                    clickedOnNode = true;
                    break;
                }
            ...
            if (!clickedOnNode) {
                if (Screen.hasShiftDown()) {
                    DiagramNode comment = new DiagramNode(UUID.randomUUID(), "creatediagram:text_comment", (int)worldX, (int)worldY, "", 1, toolbar.getCurrentColor(), 80, 60);
                    nodes.add(comment);
                    textEditor.startEditing(comment.id(), "");
                    return true;
                }
                this.isPanning = true;
            }
        ...
        if (button == 0) {
            ...
            for (int i = nodes.size() - 1; i >= 0; i--) {
                ...
                if (isComment && worldX >= node.x() + w + 2 && worldX <= node.x() + w + 18 && worldY >= node.y() + (h/2) - 8 && worldY <= node.y() + (h/2) + 8) {
                    this.nodeWithOpenColorMenu = node.id();
                    return true;
                }
                ...
                if (worldX >= node.x() && worldX <= node.x() + w && worldY >= node.y() && worldY <= node.y() + h) {
                    ...
                    if (isComment) {
                        textEditor.startEditing(node.id(), node.property());
                    }
            ...
            if (!hitNode && !toolbar.isColorMenuOpen()) {
                if (toolbar.getCurrentTool() == DrawingToolbar.Tool.PEN) {
                    currentStrokePoints = new ArrayList<>();
                    currentStrokePoints.add(new int[]{(int) worldX, (int) worldY});
                    return true;
                }
                if (toolbar.getCurrentTool() == DrawingToolbar.Tool.LINE) {
                ...
                if (toolbar.getCurrentTool() == DrawingToolbar.Tool.ERASER) {
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        ...
        if (this.connectionManager.mouseDragged(worldX, worldY, button)) return true;
        ...
        if (toolbar.getCurrentTool() == DrawingToolbar.Tool.PEN && currentStrokePoints != null) {
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (toolbar.mouseReleased(mouseX, mouseY)) return true;
        ...
        if (this.connectionManager.mouseReleased(worldX, worldY, this.nodes, button)) return true;
    ...
    private void closeAndSaveAmountField() {
    ...
    public boolean charTyped(char codePoint, int modifiers) {
        ...
        if (textEditor.isEditing()) {
            if (textEditor.charTyped(codePoint, modifiers)) {
                syncEditedComment();
            }
            return true;
        }
    ...
    private void syncEditedComment() {
        if (!textEditor.isEditing()) return;
        DiagramNode node = findNode(textEditor.getEditingNodeId());
        if (node != null) {
            int idx = nodes.indexOf(node);
            nodes.set(idx, new DiagramNode(
                    node.id(), node.itemType(), node.x(), node.y(),
                    textEditor.getCurrentText(), node.amount(), node.color(), node.width(), node.height()
            ));
        }
    }
